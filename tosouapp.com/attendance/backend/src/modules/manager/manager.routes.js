...
router.get('/report', authenticate, resolveTenant, authorize('manager','admin'), controller.groupReport);
router.post('/shifts',
  rateLimitNamed('manager_shifts', { windowMs: 60_000, max: 20 }),
  authenticate, resolveTenant, authorize('manager','admin'), controller.assignShift);
router.get('/users', authenticate, resolveTenant, authorize('manager','admin'), controller.listMyDepartment);
router.patch('/users/:id',
  rateLimitNamed('manager_users_update', { windowMs: 60_000, max: 30 }),
  authenticate, resolveTenant, authorize('manager','admin'), controller.updateEmployeeInfo);
router.get('/departments', authenticate, resolveTenant, authorize('manager','admin'), controller.listDepartments);
router.get('/salary/preview', authenticate, resolveTenant, authorize('manager','admin'), controller.salaryPreviewDepartment);
router.post('/users/:id/resign',
  rateLimitNamed('manager_users_resign', { windowMs: 60_000, max: 5 }),
  authenticate, resolveTenant, authorize('manager','admin'), controller.resignEmployee);
router.get('/profile-change/pending', authenticate, resolveTenant, authorize('manager','admin'), controller.listProfileChangePending);
router.get('/profile-change/:id', authenticate, resolveTenant, authorize('manager','admin'), controller.getProfileChange);
router.patch('/profile-change/:id/status',
  rateLimitNamed('manager_profile_change_status', { windowMs: 60_000, max: 10 }),
  authenticate, resolveTenant, authorize('manager','admin'), controller.approveProfileChange);
...
router.get('/employees/:id/photos', authenticate, resolveTenant, authorize('manager','admin'), async (req, res) => {
  try {
...
router.post('/employees/:id/photos', authenticate, resolveTenant, authorize('manager','admin'), uploadEmployeePhotos, async (req, res) => {
  try {
    await ensureEmployeeProfilePhotosSchema();
    const id = parseInt(req.params.id, 10);
    if (!id) return res.status(400).json({ message: 'Missing id' });
