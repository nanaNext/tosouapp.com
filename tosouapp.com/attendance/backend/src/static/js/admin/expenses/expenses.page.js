import{requireAdmin as Tt}from"../_shared/require-admin.js";import{fetchJSONAuth as $}from"../../api/http.api.js";import{$ as w,showSpinner as Ve,hideSpinner as _e,todayMonth as je,fmtDT as re,fmtJPY as we,fmtMonthLabel as Ke,statusLabel as ft,statusPillClass as bt,listYMBack as Ht}from"./expenses.helpers.js?v=navy-20260831-workflow1";const Rt=async ie=>{if(!ie)return;let Ae=0;const Ie=document.getElementById("status");Ie&&(Ie.textContent="",Ie.style.display="none"),ie.className="",ie.style.maxWidth="none",ie.style.width="100%",ie.style.marginLeft="0",ie.style.marginRight="0",ie.style.background="transparent",ie.style.border="0",ie.style.boxShadow="none",ie.style.padding="0",ie.innerHTML=`
    <div class="exp-admin-page">
      <style>
        ...
        .exp-admin-page .exp-dash-root.mode-settings .exp-dash-list-only,
        .exp-admin-page .exp-dash-root.mode-settings .exp-dash-dashboard-only,
        .exp-admin-page .exp-dash-root.mode-settings .exp-dash-top { display: none !important; }
        .exp-admin-page .exp-dash-root:not(.mode-settings) .exp-dash-settings-only { display: none !important; }
        .exp-admin-page .exp-dash-root.mode-overview .exp-dash-list-only,
        .exp-admin-page .exp-dash-root.mode-overview .exp-dash-dashboard-only { display: none !important; }
        .exp-admin-page .exp-dash-root:not(.mode-overview) .exp-dash-overview-only { display: none !important; }
        .exp-admin-page .exp-dash-chipbar { display: flex; flex-wrap: wrap; gap: 8px; padding: 0 0 4px; }
        .exp-admin-page .exp-dash-root.is-archived-view #expStatusChipBar { display: none !important; }
        .exp-admin-page .exp-dash-chipbar .exp-dash-nav.exp-dash-chip {
          display: inline-flex; align-items: center; gap: 6px; width: auto;
          background: #fff; border: 1px solid #dbe3ee; color: #475569;
          border-radius: 999px; padding: 6px 14px; font-size: 12px; font-weight: 700;
          cursor: pointer; white-space: nowrap;
        }
        .exp-admin-page .exp-dash-chipbar .exp-dash-nav.exp-dash-chip::before { display: none; }
        .exp-admin-page .exp-dash-chipbar .exp-dash-nav.exp-dash-chip:hover { background: #f8fafc; }
        .exp-admin-page .exp-dash-chipbar .exp-dash-nav.exp-dash-chip.is-active { background: #eef2ff; border-color: #6366f1; color: #3730a3; }
        .exp-admin-page .exp-dash-chipbar .exp-dash-badge { position: static; margin-left: 2px; }
        .exp-admin-page .exp-overview-pill { display:inline-flex; align-items:center; padding:4px 12px; border-radius:999px; font-size:12px; font-weight:700; white-space:nowrap; }
        .exp-admin-page .exp-set-input { width: 100%; box-sizing: border-box; }
        .exp-admin-page #expSetMileageTiers td { vertical-align: middle; }
        .exp-admin-page .exp-set-row-remove { border: none; background: transparent; color: #94a3b8; cursor: pointer; font-size: 14px; }
        .exp-admin-page .exp-set-row-remove:hover { color: #ef4444; }
        .exp-admin-page .exp-dash-bulk-action-btn[disabled] { opacity: .4; cursor: not-allowed; filter: grayscale(.4); }
        .exp-admin-page .exp-dash-nav-group-toggle {
          width: calc(100% - 0px); display: flex; align-items: center; justify-content: space-between;
          background: transparent; border: none; cursor: pointer; font-family: inherit;
          font-size: 11px; color: #64748b; font-weight: 800; padding: 16px 20px 4px;
        }
        .exp-admin-page .exp-dash-nav-group-toggle:hover { color: #cbd5e1; }
        .exp-admin-page .exp-dash-nav-group-toggle .chevron { transition: transform .15s ease; flex: none; }
        .exp-admin-page .exp-dash-nav-group-toggle[aria-expanded="true"] .chevron { transform: rotate(180deg); }
        .exp-admin-page .exp-dash-root.collapsed .exp-dash-nav-group-toggle { display: none; }
        .exp-admin-page .exp-dash-root.collapsed .exp-dash-nav-group { display: block !important; }
        .exp-admin-page .exp-dash-empty-state { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; padding: 48px 16px; color: #94a3b8; }
        .exp-admin-page .exp-dash-empty-state svg { color: #cbd5e1; }
        .exp-admin-page .exp-dash-empty-state .msg { font-size: 13px; font-weight: 700; color: #64748b; }
        ...
        .exp-admin-page .exp-dash-badge[hidden] { display: none !important; }
      ...
      <section id="expDashRoot" class="exp-dash-root">
        <aside class="exp-dash-side">
          ...
          <div class="exp-dash-side-nav-wrapper">
            ...
            <button id="expNavClaims" type="button" class="exp-dash-nav">
              <span class="left">
                <span class="ico">
              ...
              <span id="expBadgeTodo" class="exp-dash-badge" hidden>0</span>
            </button>
            <button id="expNavOverview" type="button" class="exp-dash-nav">
              <span class="left">
                <span class="ico">
            ...
            <div style="font-size: 11px; color: #64748b; font-weight: 800; padding: 16px 20px 4px;">\u8A2D\u5B9A</div>
            <button id="expNavSettings" type="button" class="exp-dash-nav">
              <span class="left">
                <span class="ico">
        ...
        <main class="exp-dash-main">
          ...
          <div id="expStatusChipBar" class="exp-dash-chipbar exp-dash-list-only">
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="">
              <span>すべて</span>
            </button>
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="applied">
              <span>未承認（申請中）</span>
              <span id="expBadgeApplied" class="exp-dash-badge" hidden>0</span>
            </button>
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="soumu_checked">
              <span>総務確認済（承認待ち）</span>
              <span id="expBadgeSoumu" class="exp-dash-badge" hidden>0</span>
            </button>
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="approved">
              <span>未支給（承認済）</span>
              <span id="expBadgeApproved" class="exp-dash-badge" hidden>0</span>
            </button>
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="paid">
              <span>支給済み</span>
              <span id="expBadgePaid" class="exp-dash-badge" hidden>0</span>
            </button>
            <button type="button" class="exp-dash-nav exp-dash-chip" data-status="rejected">
              <span>差戻し</span>
          ...
          <div class="exp-dash-card exp-dash-list-only">
            ...
            <div id="expOverviewPanel" class="exp-dash-overview-only exp-dash-card">
              <div class="exp-dash-card-head">
                <div class="exp-dash-card-title">未申請チェック（在籍全員）</div>
                <div id="expOverviewMeta" class="exp-dash-muted"></div>
              </div>
              <div class="exp-dash-tablewrap">
                <table class="exp-dash-table">
                  <thead><tr><th>社員名</th><th>部署</th><th class="center">件数</th><th class="money">合計金額</th><th class="center">ステータス</th></tr></thead>
                  <tbody id="expOverviewList"></tbody>
                </table>
              </div>
            </div>
            <div id="expSettingsPanel" class="exp-dash-settings-only">
              <div style="font-size: 22px; font-weight: 800; color: #0f172a; margin-bottom: 4px;">設定</div>
              <div style="font-size: 13px; color: #64748b; margin-bottom: 16px;">非課税限度額の基準・期限アラートの日数を調整できます</div>
              <div class="exp-dash-card">
                <div class="exp-dash-card-title" style="margin-bottom: 16px;">非課税限度額の基準</div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 20px; max-width: 640px; margin-bottom: 24px;">
                  <label style="display:flex; flex-direction:column; gap:6px; font-size:12px; color:#334155; font-weight:700;">
                    電車・バス通勤（月額上限・円）
                    <input id="expSetCommuteLimit" type="number" min="0" step="1" class="exp-set-input" style="padding:8px 10px; border:1px solid #dbe3ee; border-radius:6px; font-size:14px;">
                  </label>
                  <label style="display:flex; flex-direction:column; gap:6px; font-size:12px; color:#334155; font-weight:700;">
                    期限アラート（何日前から表示するか）
                    <input id="expSetAlertDays" type="number" min="0" step="1" class="exp-set-input" style="padding:8px 10px; border:1px solid #dbe3ee; border-radius:6px; font-size:14px;">
                  </label>
                </div>
                <div style="font-size:13px; font-weight:800; color:#0f172a; margin-bottom: 8px;">マイカー等通勤の片道距離別 非課税額（円） — 法定の目安値</div>
                <table class="exp-admin-table clean-view" style="max-width: 640px;">
                  <thead><tr><th>片道距離</th><th>非課税額</th><th style="width:40px;"></th></tr></thead>
                  <tbody id="expSetMileageTiers"></tbody>
                </table>
                <button id="expSetMileageAddRow" type="button" class="btn" style="margin-top:10px; font-size:12px; padding:6px 12px;">+ 行を追加</button>
                <div style="margin-top:24px; display:flex; align-items:center; gap:12px;">
                  <button id="expSetSave" type="button" class="btn exp-dash-btn-approve" style="padding:10px 24px;">保存する</button>
                  <span id="expSetStatus" style="font-size:13px; color:#64748b;"></span>
                </div>
              </div>
            </div>
            </div>
        ...
        <div class="exp-admin-actions">
          ...
          <details class="exp-admin-actions-more">
            ...
            <div class="exp-admin-more-pop">
              ...
              <button id="expExportXlsx" class="btn exp-admin-btn-secondary" type="button" style="color:#166534;border-color:#166534;">Excel\u51FA\u529B</button>
    ...
    `},Qe=()=>{try{return!!(window.matchMedia&&window.matchMedia("(min-width: 1101px)").matches)}catch{return!1}},Be=async i=>{const s=document.getElementById("expDashDrawer"),e=document.getElementById("expDashBackdrop"),t=document.getElementById("expDashDrawerBody");if(!s||!t)return;try{const a=document.getElementById("expDashRoot");a&&Qe()&&a.classList.add("with-drawer")}catch{}o.selectedId=String(i||""),s.removeAttribute("hidden");const n=Qe();try{n?(e?.setAttribute("hidden",""),document.body.style.overflow=""):(e?.removeAttribute("hidden"),document.body.style.overflow="hidden")}catch{}t.innerHTML='<div class="exp-dash-muted">\u8AAD\u307F\u8FBC\u307F\u4E2D\u2026</div>';try{const[a,c]=await Promise.all([$(`/api/expenses/admin/detail/${encodeURIComponent(o.selectedId)}`),$(`/api/expenses/${encodeURIComponent(o.selectedId)}/files`).catch(()=>[])]),r=[a?.origin||"",a?.destination||""].filter(Boolean).join(" \u2192 ")||"-",d=a?.departmentId?String(a.departmentId):"",l=d?o.deptMap.get(d)||`#${d}`:"",m=Array.isArray(c)?c:[],u=m.length?m.map(Q=>{const X=String(Q?.path||""),be=X.startsWith("/")?X:"/"+X,se=Q?.name||be.split("/").pop();return`
      ...
      `;const P=document.getElementById("expDashPayMethodSave");P&&!P.dataset.bound&&(P.dataset.bound="1",P.addEventListener("click",async()=>{const Q=document.getElementById("expDashPayMethod"),X=document.getElementById("expDashPayMethodMsg"),be=String(Q?.value||"");P.disabled=!0;try{await $(`/api/expenses/${encodeURIComponent(o.selectedId)}`,{method:"PATCH",body:JSON.stringify({payment_method:be})}),X&&(X.style.color="#16a34a",X.textContent="\u652F\u6255\u65B9\u6CD5\u3092\u4FDD\u5B58\u3057\u307E\u3057\u305F\u3002")}catch(se){X&&(X.style.color="#b00020",X.textContent=String(se?.message||"\u4FDD\u5B58\u306B\u5931\u6557\u3057\u307E\u3057\u305F"))}finally{P.disabled=!1}}));const U=String(window.ADMIN_PROFILE&&window.ADMIN_PROFILE.role||"").toLowerCase(),Y=U==="admin"||U==="owner"||U==="sysadmin",p=U==="manager"||Y,g=String(x).toLowerCase();let b="",A="";g==="applied"&&p?(b="soumu_checked",A="\u7DCF\u52D9\u78BA\u8A8D\u3059\u308B"):g==="soumu_checked"&&Y?(b="approved",A="\u793E\u9577\u627F\u8A8D\u3059\u308B"):g==="approved"&&Y&&(b="paid",A="\u652F\u7D66\u3059\u308B");const M=["applied","soumu_checked","approved"].includes(g)&&p,L=!!b||M,B=document.querySelector(".exp-dash-drawer-foot");B&&(B.style.display=L?"grid":"none");const V=document.getElementById("expDashApprove");V&&(b?(V.style.display="",V.textContent=A,V.setAttribute("data-next",b)):(V.style.display="none",V.setAttribute("data-next","")));const ae=document.getElementById("expDashReject");ae&&(ae.style.display=M?"":"none");const _=document.getElementById("expDashNote");_&&(_.value="")}catch(a){t.innerHTML=`<div class="exp-dash-muted" style="color:#b00020;">${y(String(a?.message||"\u8AAD\u307F\u8FBC\u307F\u306B\u5931\u6557\u3057\u307E\u3057\u305F"))}</div>`}},He=()=>{const i=document.getElementById("expDashDrawer"),s=document.getElementById("expDashBackdrop");i&&i.setAttribute("hidden",""),s&&s.setAttribute("hidden",""),o.selectedId="";try{document.getElementById("expDashRoot")?.classList.remove("with-drawer")}catch{}try{document.body.style.overflow=""}catch{}},Xe=i=>{const s=document.getElementById("expDashList"),e=document.getElementById("expDashListMeta"),t=document.getElementById("expDashTopTotal");if(!s)return;const n=Array.isArray(i?.rows)?i.rows:[],a=new Map;n.forEach(p=>{const g=String(p.userId||"");if(!g)return;const b=String(p.date||"").slice(0,7),A=String(p.status||"pending").toLowerCase(),M=`${g}_${b}`,L=a.get(M)||{userId:g,userName:p.user_name||p.user_email||"\u793E\u54E1",userCode:p.employee_code,dept:p.departmentId,count:0,amount:0,month:b,items:[]};L.count+=1,L.amount+=Number(p.amount||0),L.items.push(p),a.set(M,L)});const c=p=>{const g=p==null?"":String(p);return g?o.deptMap.get(g)||`#${g}`:"\u672A\u8A2D\u5B9A"},r=Array.from(a.values()),d=r.reduce((p,g)=>p+Number(g.amount||0),0),l=r.reduce((p,g)=>p+Number(g.count||0),0),m=o.status==="archived",u=r.map(p=>{const g=c(p.dept),b=new URLSearchParams(window.location.search).get("standalone");let A=`/admin/expenses/monthly-detail?userId=${encodeURIComponent(p.userId)}&month=${encodeURIComponent(p.month)}`;b&&(A+=`&standalone=${encodeURIComponent(b)}&tab=${encodeURIComponent(o.status)}`);const M=o.status==="monthly_approval"||o.status==="archived",L=o.status==="applied";let B="";if(L)B=`<button type="button" class="btn exp-dash-btn-ghost" data-action="toggle-dash-group" data-target="grp_${p.userId}_${p.month}" style="min-height:28px;padding:0 8px;font-size:12px;border:1px solid #cbd5e1;background:#fff;border-radius:4px;display:inline-flex;align-items:center;color:#0f172a;">\u660E\u7D30</button>`;else{if(B=`<div style="display:flex;gap:4px;align-items:center;">
        ...
        ${o.status==="approved"?"":`<td data-label="" class="center" style="width:40px;">
          <input type="checkbox" class="${o.status==="monthly_approval"?"exp-dash-bulk-cb-monthly":"exp-dash-bulk-group"}" data-uid="${y(p.userId)}" data-month="${y(p.month)}" data-ids="${y(p.items.map(_=>_.id).join(","))}" style="cursor:pointer;" />
        </td>`}
        <td data-label="\u793E\u54E1\u540D">
      ...
      </tr>`+V}).join("");e&&(e.textContent=`${r.length} \u540D\u306E${m?"\u30C7\u30FC\u30BF":"\u7533\u8ACB"}`),t&&(t.innerHTML=`<span style="display:inline-flex; align-items:center; gap:6px; background:#f1f5f9; color:#334155; font-weight:700; padding:4px 12px; border-radius:999px; margin-right:8px;">\u5BFE\u8C61\u793E\u54E1 <b style="color:#0f172a;">${r.length}\u540D</b></span><span style="display:inline-flex; align-items:center; gap:6px; background:#eef2ff; color:#4338ca; font-weight:700; padding:4px 12px; border-radius:999px;">\u5408\u8A08\u91D1\u984D <b style="color:#0f172a;">\xA5${d.toLocaleString("ja-JP")}</b></span>`,m&&(t.innerHTML+=' <span style="margin-left:16px; color:#10b981; font-weight:800; border: 1px solid #10b981; padding: 2px 8px; border-radius: 4px; font-size: 11px;">\u6708\u6B21\u7DE0\u3081\u5B8C\u4E86</span>'),t.removeAttribute("hidden"));const x='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>',f='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',h='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>';let E="";if(o.status==="monthly_approval")E=`
      <div style="margin-bottom: 8px; display: flex; gap: 8px; align-items: center; padding: 0 12px; justify-content: space-between; flex-wrap: wrap;">
        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
          ...
          <button type="button" class="btn exp-dash-bulk-action-btn" id="expDashBulkApproveMonthly" disabled style="display:flex; align-items:center; background:#10b981; color:#fff; border:none; padding:4px 12px; font-size:12px; border-radius:4px; font-weight:bold;">
            ${x} \u4E00\u62EC\u627F\u8A8D (Approve)
          </button>
          <button type="button" class="btn exp-dash-bulk-action-btn" id="expDashBulkCancelMonthly" disabled style="display:flex; align-items:center; background:#fff; color:#475569; border:1px solid #cbd5e1; padding:4px 12px; font-size:12px; border-radius:4px; font-weight:bold;">
            ${f} \u4E00\u62EC\u53D6\u6D88
          </button>
        </div>
      ...
      <div style="margin-bottom: 8px; display: flex; gap: 8px; align-items: center; padding: 0 12px; flex-wrap: wrap;">
        ...
        <button type="button" class="btn exp-dash-bulk-action-btn" id="expDashBulkApprove" disabled style="display:flex; align-items:center; background:#10b981; color:#fff; border:none; padding:4px 12px; font-size:12px; border-radius:4px; font-weight:bold;">${x}${p}</button>
        <button type="button" class="btn exp-dash-bulk-action-btn" id="expDashBulkCancel" disabled style="display:flex; align-items:center; background:#fff; color:#475569; border:1px solid #cbd5e1; padding:4px 12px; font-size:12px; border-radius:4px; font-weight:bold;">${f}\u4E00\u62EC\u53D6\u6D88</button>
        <button type="button" class="btn exp-dash-bulk-action-btn" id="expDashBulkDelete" disabled style="display:flex; align-items:center; background:#fff; color:#ef4444; border:1px solid #fecaca; padding:4px 12px; font-size:12px; border-radius:4px; font-weight:bold;">${h}\u4E00\u62EC\u524A\u9664</button>
      </div>
      ...
      <div class="exp-dash-tablewrap">
        <table class="exp-dash-table">
          ...
          <tbody>${u||`<tr><td colspan="${o.status==="approved"?"6":"7"}" style="padding:0;"><div class="exp-dash-empty-state"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="9" y1="13" x2="15" y2="13"></line><line x1="9" y1="17" x2="15" y2="17"></line></svg><span class="msg">\u5BFE\u8C61\u306E\u30C7\u30FC\u30BF\u306F\u3042\u308A\u307E\u305B\u3093</span></div></td></tr>`}</tbody>
          ${r.length>0?`
          <tfoot>
...
function initExpOverviewPanel() {
  ...
  document.getElementById('expDashMonth')?.addEventListener('change', () => {
    if (root?.classList.contains('mode-overview')) loadOverview();
  });

  document.querySelectorAll('.exp-dash-side .exp-dash-nav[data-nav], .exp-dash-nav[data-status], #expNavSettings').forEach(btn => {
    btn.addEventListener('click', () => { root?.classList.remove('mode-overview'); });
  });
}
async function refreshExpBadges() {
  // 一括承認/支給など一覧画面からの操作後は Se() しか呼ばれず、
  // サイドバーのバッジ(未承認/総務確認済み/未支給など)は ge() (Home読み込み)でしか更新されていなかった。
  // そのため支給済みにしてもサイドバーの数字が古いままになっていた。
  try {
    const n = await yt();
    Te('expBadgeApplied', n?.month?.appliedCount || 0);
    Te('expBadgeSoumu', n?.month?.soumuCheckedCount || 0);
    Te('expBadgeApproved', n?.month?.approvedCount || 0);
    Te('expBadgeRejected', n?.month?.rejectedCount || 0);
    Te('expBadgeTodo', Number(n?.month?.appliedCount || 0) + Number(n?.month?.soumuCheckedCount || 0));
    Te('expDashBellBadge', n?.month?.appliedCount || 0);
  } catch {}
}
function initExpBulkButtonsUX() {
  function countSelected() {
    return document.querySelectorAll('.exp-dash-bulk-cb:checked, .exp-dash-bulk-group:checked, .exp-dash-bulk-cb-monthly:checked').length;
  }
  function refresh() {
    const n = countSelected();
    document.querySelectorAll('.exp-dash-bulk-action-btn').forEach(btn => { btn.disabled = n === 0; });
  }
  // 行クリック経由の選択(stopPropagation や bubbles:false の change イベントを使う箇所がある)も
  // 確実に拾うため、capture フェーズで document 全体を監視し、
  // 元のクリック/change 処理が完全に終わった後(setTimeout)に再評価する。
  document.addEventListener('click', () => { setTimeout(refresh, 0); }, true);
  document.addEventListener('change', () => { setTimeout(refresh, 0); }, true);
  const list = document.getElementById('expDashList');
  if (list) {
...
async function initExpSettingsPanel() {
  const panel = document.getElementById('expSettingsPanel');
  const navBtn = document.getElementById('expNavSettings');
  if (!panel || !navBtn) return;
  const root = document.getElementById('expDashRoot');
  const commuteInput = document.getElementById('expSetCommuteLimit');
  const alertInput = document.getElementById('expSetAlertDays');
  const tbody = document.getElementById('expSetMileageTiers');
  const addRowBtn = document.getElementById('expSetMileageAddRow');
  const saveBtn = document.getElementById('expSetSave');
  const statusEl = document.getElementById('expSetStatus');

  function renderTierRow(tier) {
    const tr = document.createElement('tr');
    const minVal = tier && tier.minKm != null ? tier.minKm : 0;
    const maxVal = tier && tier.maxKm != null ? tier.maxKm : '';
    const amountVal = tier && tier.taxFreeAmount != null ? tier.taxFreeAmount : 0;
    tr.innerHTML = `
      <td>
        <input type="number" class="exp-set-input tier-min" min="0" step="0.1" value="${y(minVal)}" style="width:80px; display:inline-block;"> km 〜
        <input type="number" class="tier-max" min="0" step="0.1" value="${y(maxVal)}" placeholder="上限なし" style="width:90px; display:inline-block;"> km未満
      </td>
      <td><input type="number" class="tier-amount" min="0" step="1" value="${y(amountVal)}" style="width:110px;"> 円</td>
      <td><button type="button" class="exp-set-row-remove" title="削除">✕</button></td>
    `;
    const removeBtn = tr.querySelector('.exp-set-row-remove');
  ...
  function renderTiers(tiers) {
    tbody.innerHTML = '';
    (Array.isArray(tiers) ? tiers : []).forEach(t => tbody.appendChild(renderTierRow(t)));
  }

  function collectTiers() {
    return Array.from(tbody.querySelectorAll('tr')).map(tr => {
      const minKm = Number(tr.querySelector('.tier-min')?.value || 0);
      const maxRaw = String(tr.querySelector('.tier-max')?.value ?? '').trim();
      const maxKm = maxRaw === '' ? null : Number(maxRaw);
      const taxFreeAmount = Number(tr.querySelector('.tier-amount')?.value || 0);
      return { minKm, maxKm, taxFreeAmount };
  ...
  async function loadSettings() {
    ...
    try {
      const data = await $('/api/expenses/admin/settings');
      commuteInput.value = Number(data?.commuteAllowanceTaxFreeLimit || 150000);
      alertInput.value = Number(data?.deadlineAlertDays ?? 30);
      renderTiers(data?.mileageTiers || []);
      statusEl.textContent = '';
    } catch (e) {
      statusEl.style.color = '#b00020';
      statusEl.textContent = String(e?.message || '取得に失敗しました');
    }
  }

  addRowBtn?.addEventListener('click', () => {
    tbody.appendChild(renderTierRow({ minKm: 0, maxKm: null, taxFreeAmount: 0 }));
  });
  ...
  saveBtn?.addEventListener('click', async () => {
    ...
    try {
      await $('/api/expenses/admin/settings', {
        method: 'POST',
        body: JSON.stringify({
          commuteAllowanceTaxFreeLimit: Number(commuteInput.value || 0),
          deadlineAlertDays: Number(alertInput.value || 0),
          mileageTiers: collectTiers()
        })
      });
      statusEl.style.color = '#16a34a';
      statusEl.textContent = '保存しました';
      setTimeout(() => { if (statusEl.textContent === '保存しました') statusEl.textContent = ''; }, 3000);
    } catch (e) {
      statusEl.style.color = '#b00020';
      statusEl.textContent = String(e?.message || '保存に失敗しました');
    } finally {
  ...
  navBtn.addEventListener('click', async () => {
    document.querySelectorAll('.exp-dash-side .exp-dash-nav').forEach(n => n.classList.remove('is-active'));
    navBtn.classList.add('is-active');
    root?.classList.add('mode-settings');
    const titleEl = document.getElementById('expDashTitle');
    if (titleEl) titleEl.textContent = '設定';
    if (window.innerWidth <= 760) {
      root?.classList.remove('mobile-open');
      document.getElementById('expDashBackdrop')?.setAttribute('hidden', '');
      document.body.style.overflow = '';
  ...
  // 他のナビ(dashboard/ステータス別)をクリックしたら設定モードを解除する
  document.querySelectorAll('.exp-dash-side .exp-dash-nav[data-nav], .exp-dash-nav[data-status], #expNavOverview').forEach(btn => {
    btn.addEventListener('click', () => { root?.classList.remove('mode-settings'); });
  });
}
try{const _mu=new URLSearchParams(window.location.search).get("month");if(_mu&&/^\d{4}-\d{2}$/.test(_mu)){o.month=_mu;const _mi=document.getElementById("expDashMonth");_mi&&(_mi.value=_mu)}}catch{}
const Ze=new URLSearchParams(window.location.search).get("tab");if(Ze){const i=document.querySelector(`.exp-dash-nav[data-status="${Ze}"]`);if(i){document.querySelectorAll(".exp-dash-nav").forEach(n=>n.classList.remove("is-active")),i.classList.add("is-active");const s=String(i.getAttribute("data-status")||"");String(i.getAttribute("data-nav")||"")==="dashboard"?(o.view="dashboard",o.status=""):(o.view="list",o.status=s);const t=n=>{const a=document.getElementById("expDashTitle");a&&(a.textContent=n)};o.status==="applied"?t("\u627F\u8A8D\u7BA1\u7406"):o.status==="soumu_checked"?t("\u7DCF\u52D9\u78BA\u8A8D\u6E08\u307F\uFF08\u793E\u9577\u627F\u8A8D\u5F85\u3061\uFF09"):o.status==="monthly_approval"||o.status==="applied_approved"?t("\u6708\u6B21\u627F\u8A8D"):o.status==="approved"?t("\u627F\u8A8D\u6E08\u307F"):o.status==="rejected"?t("\u5DEE\u623B\u3057\u4E00\u89A7"):t("\u7533\u8ACB\u4E00\u89A7")}}initExpHistoryGroupToggle();function initExpHistoryGroupToggle(){const toggle=document.getElementById("expNavHistoryToggle"),group=document.getElementById("expNavHistoryGroup");if(!toggle||!group)return;let expanded=!!group.querySelector(".exp-dash-nav.is-active");if(!expanded){try{expanded=localStorage.getItem("expHistoryGroupExpanded")==="1"}catch{}}const apply=()=>{group.hidden=!expanded,toggle.setAttribute("aria-expanded",expanded?"true":"false")};apply();toggle.addEventListener("click",()=>{expanded=!expanded;apply();try{localStorage.setItem("expHistoryGroupExpanded",expanded?"1":"0")}catch{}})}return Ee(),await ge(),()=>{try{Ae&&window.clearInterval(Ae)}catch{}try{_e()}catch{}try{He()}catch{}};const $t=(i,s)=>{const e=String(i||"").toLowerCase();return s==="approved"?e==="approved":s==="applied_approved"?e==="approved"||e==="applied":["approved","applied","rejected","pending","draft"].includes(e)},Je=i=>i==="approved"?"\u627F\u8A8D":i==="applied_approved"?"\u7533\u8ACB+\u627F\u8A8D":"\u5168\u72B6\u614B",At=i=>{const s=w("#expMonthlyKpiHost");if(!s)return;const e=Array.isArray(i)?i:[],t={approved:{count:0,amount:0},applied:{count:0,amount:0},rejected:{count:0,amount:0}};for(const a of e){const c=String(a.status||"").toLowerCase();t[c]&&(t[c].count+=1,t[c].amount+=Number(a.amount||0))}const n=(a,c,r,d)=>`
