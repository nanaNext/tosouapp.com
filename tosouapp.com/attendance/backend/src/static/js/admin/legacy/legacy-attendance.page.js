import{escapeHtml as v,delegate as Pt}from"../_shared/dom.js";import{api as vt,downloadWithAuth as Z}from"../../shared/api/client.js";import{createPage as Wt}from"../../shared/page/createPage.js";import{createCleanup as Xt}from"../../shared/page/createCleanup.js";async function Kt(F){return await kt(F)}async function kt({content:F,listUsers:tt,getTimesheet:Et,getAttendanceDay:Ft,updateAttendanceSegment:Gt,buildTimesheetExportURL:et}){const _=Xt();let B=!0;const ot=new AbortController,N=ot.signal;_.add(()=>{B=!1}),_.add(()=>ot.abort());let rt=[];try{if(!window.location.pathname.includes("/ui/attendance-records")&&typeof tt=="function"){const t=await tt({signal:N});rt=Array.isArray(t)?t:t&&Array.isArray(t.rows)?t.rows:[]}}catch(t){console.warn("Could not fetch users for dropdown:",t)}F.innerHTML="";const nt=t=>{if(!t)return"";const e=String(t);return e.length>=16?e.slice(11,16):e},Ct=t=>{try{const e=String(t||"").slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(e))return!1;const[r,i,o]=e.split("-").map(d=>parseInt(d,10)),a=new Date(Date.UTC(r,i-1,o)).getUTCDay();return a===0||a===6}catch{return!1}},z=new Date(Date.now()+9*3600*1e3).toISOString().slice(0,10),Dt=z.slice(0,7),it=new URLSearchParams(window.location.search).get("standalone")==="1"||window.location.pathname.includes("/ui/attendance-records");if(it)try{const t=document.querySelector(".topbar"),e=document.querySelector(".subbar");t&&(t.style.display="none"),e&&(e.style.display="none");const r=document.querySelector("#adminChrome");r&&(r.style.display="none"),document.body.style.paddingTop="0";const i=document.documentElement;i.style.setProperty("--topbar-height","0px"),i.style.setProperty("--subbar-height","0px")}catch{}const Jt=it?"100vh":"calc(100vh - var(--topbar-height) - var(--subbar-height))",b=document.createElement("div");b.style.cssText="margin: 0; padding: 0; width: 100%; display: flex; flex-direction: column;",b.innerHTML=`
    ...
    `;if(window.innerWidth<=768)if(at)at.innerHTML=lt;else{const t=document.createElement("div");t.id="rosterMobileDateBar",t.style.cssText="display:flex; align-items:center; justify-content:center; padding:8px 12px; background:#fff; border-bottom:1px solid #e5e7eb;",t.innerHTML=lt,b.insertBefore(t,b.firstChild)}const St=`
        <div class="attrec-control hidden-on-mobile" style="display: flex !important; flex-direction: row !important; align-items: center !important; justify-content: space-between !important; width: 100%; padding: 0 !important; gap: 12px; background: transparent !important; border: none !important; overflow: visible !important;">
          <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
            ...
            <select id="rosterFilterDept" style="height:34px;border:1px solid #cbd5e1;border-radius:0;padding:0 8px;font-size:12px;color:#334155;background:#fff;cursor:pointer;min-width:90px;"><option value="">\u5168\u90E8\u90E8\u7F72</option></select>
            <select id="rosterFilterStatus" style="height:34px;border:1px solid #cbd5e1;border-radius:0;padding:0 8px;font-size:12px;color:#334155;background:#fff;cursor:pointer;min-width:90px;"><option value="">\u5168\u72B6\u614B</option><option value="working">\u51FA\u52E4\u4E2D</option><option value="checked_out">\u9000\u52E4\u6E08</option><option value="absent">\u672A\u51FA\u52E4</option><option value="late">\u9045\u523B</option><option value="off">\u4F11\u65E5</option><option value="leave">\u4F11\u6687</option></select>
  ...
  `,T=b.querySelector(".attrec-controls");T&&(T.innerHTML=St),window.addEventListener("resize",()=>{const t=window.innerWidth<=768;T&&(T.style.display=t?"none":"block")}),T&&(T.style.display=window.innerWidth<=768?"none":"block");const dt=t=>{const c=b.querySelector("#rosterSummaryInline");if(!c)return;const sm=t&&typeof t==="object"?t:{};const working=Number(sm.working||0),checked_out=Number(sm.checked_out||0),absent=Number(sm.absent||0),off=Number(sm.off||0),leave=Number(sm.leave||0),late=Number(sm.late||0);const pill=(txt,bg,color,border,count)=>count>0?`<span style="display:inline-flex;align-items:center;gap:4px;padding:2px 10px;border-radius:4px;font-size:12px;font-weight:700;background:${bg};color:${color};border:1px solid ${border};">${txt} <span style="background:rgba(0,0,0,0.12);border-radius:2px;padding:0 5px;font-size:11px;">${count}</span></span>`:"";c.style.display="flex";c.style.gap="6px";c.style.flexWrap="wrap";c.style.alignItems="center";c.innerHTML=[pill("出勤中","#fef9c3","#854d0e","#fde047",working),pill("退勤済","#dcfce7","#166534","#86efac",checked_out),pill("遅刻","#fff7ed","#c2410c","#fed7aa",late),pill("未出勤","#fef2f2","#991b1b","#fecaca",absent),pill("休日・休暇","#f1f5f9","#475569","#cbd5e1",off+leave)].filter(Boolean).join(" ");const summaryInline=b.querySelector("#rosterSummaryInline");if(summaryInline&&summaryInline!==c){summaryInline.style.display="flex";summaryInline.style.gap="6px";summaryInline.style.flexWrap="wrap";summaryInline.innerHTML=c.innerHTML;}},O=async t=>{const e=b.querySelector("#rosterTable");e&&(e.innerHTML=`
        <div class="empty-state">
          <div style="font-size:28px;">\u23F3</div>
          <div>\u8AAD\u307F\u8FBC\u307F\u4E2D\u2026</div>
        </div>
      `),dt(null);try{const r=await vt.get(`/api/admin/work-reports?date=${encodeURIComponent(t)}`,{signal:N});if(!B)return;let i=r&&Array.isArray(r.items)?r.items:[];{const _seen=new Set();i=i.filter(_it=>{const _sig=[_it.userId,_it.status,_it.attendance&&_it.attendance.checkIn,_it.attendance&&_it.attendance.checkOut,_it.report&&_it.report.site,_it.report&&_it.report.work,_it.dailyKubun,_it.workType].join("|");if(_seen.has(_sig))return false;_seen.add(_sig);return true;});}const nowJST=new Date(Date.now()+9*3600*1e3);const nowHHMM=nowJST.getHours().toString().padStart(2,"0")+":"+nowJST.getMinutes().toString().padStart(2,"0");const dateIsToday=t===z;const LATE_THRESHOLD="09:00";const computedSummary={working:0,checked_out:0,absent:0,off:0,leave:0,late:0};for(const _it of i){const _p=_it.status||"";const _ck=_it.attendance&&_it.attendance.checkIn?String(_it.attendance.checkIn).slice(11,16):"";if(_p==="working"||_p==="holiday_working")computedSummary.working++;else if(_p==="checked_out"||_p==="holiday_work")computedSummary.checked_out++;else if(_p==="off"||_p==="not_checked_in"&&(String(_it.dailyKubun||"").includes("\u4F11")))computedSummary.off++;else if(_p==="leave")computedSummary.leave++;else computedSummary.absent++;if(_ck&&_ck>LATE_THRESHOLD)computedSummary.late++;}if(dt(computedSummary),!e)return;if(!i.length){e.innerHTML=`
          <div class="empty-state">
            <div style="font-size:28px;">\u{1F5C2}\uFE0F</div>
            <div>\u30C7\u30FC\u30BF\u304C\u3042\u308A\u307E\u305B\u3093</div>
          </div>
        `;return}let o=1;const a=window.innerWidth<=768,d=999999,u=()=>{if(!e)return;e.innerHTML="";const m=document.createElement("table");m.id="attrecList",m.className="beautiful-table",m.style.tableLayout="auto",m.style.width="auto",m.style.minWidth="100%",m.style.borderCollapse="collapse",m.style.border="none",m.style.borderRadius="0",m.style.overflow="hidden",m.innerHTML=`
          <style>
            .beautiful-table {
              box-shadow: none;
            }
        ...
        `;const I=document.createElement("tbody"),k=Ct(t),S=t<z;const _fDept=b.querySelector("#rosterFilterDept")?.value||"";const _fStatus=b.querySelector("#rosterFilterStatus")?.value||"";const LATE_THR="09:00";const iFiltered=i.filter(_row=>{const _p=_row.status||"";const _ck=_row.attendance&&_row.attendance.checkIn?String(_row.attendance.checkIn).slice(11,16):"";const _dept=_row.departmentName||"";if(_fDept&&_dept!==_fDept)return false;if(_fStatus){if(_fStatus==="late")return!!(_ck&&_ck>LATE_THR);if(_fStatus==="absent")return _p!=="working"&&_p!=="checked_out"&&_p!=="off"&&_p!=="leave"&&_p!=="holiday_working"&&_p!=="holiday_work";if(_fStatus==="working")return _p==="working"||_p==="holiday_working";if(_fStatus==="checked_out")return _p==="checked_out"||_p==="holiday_work";if(_fStatus==="off")return _p==="off";if(_fStatus==="leave")return _p==="leave";return _p===_fStatus;}return true;});const _depts=[...new Set(i.map(r=>r.departmentName||"").filter(Boolean))].sort();const _dSel=b.querySelector("#rosterFilterDept");if(_dSel&&_dSel.options.length<=1){_depts.forEach(d=>{const op=document.createElement("option");op.value=d;op.textContent=d;_dSel.appendChild(op);});}const L=(o-1)*d,K=Math.min(L+d,iFiltered.length),H=iFiltered.slice(L,K);for(let w=0;w<H.length;w++){const n=H[w],f=w>0?H[w-1]:null,x=f&&f.userId===n.userId;let $="";if(!x){let R=1;for(let j=w+1;j<H.length&&H[j].userId===n.userId;j++)R++;$=R>1?` rowspan="${R}"`:""}const V=n.employeeCode||`EMP${String(n.userId).padStart(3,"0")}`,A=n.username||"",G=n.departmentName||"",p=n.status||"",c=String(n.dailyKubun||"").trim()||(k&&(p==="leave"||p==="off")?"\u4F11\u65E5":""),ht=new Set(["\u6B20\u52E4","\u6709\u7D66\u4F11\u6687","\u534A\u4F11","\u7121\u7D66\u4F11\u6687"]),At=new Set(["\u4F11\u65E5","\u4EE3\u66FF\u4F11\u65E5","\u4F11\u65E5\u4E88\u5B9A"]),zt=new Set(["\u6B20\u52E4","\u6709\u7D66\u4F11\u6687","\u534A\u4F11","\u7121\u7D66\u4F11\u6687","\u4F11\u65E5","\u4EE3\u66FF\u4F11\u65E5","\u4F11\u65E5\u4E88\u5B9A"]),U=At.has(c);let g="",h="",y="";U?(g=c||"\u4F11\u65E5",h="attrec-pill neutral",y=c==="\u534A\u4F11(\u6709\u7D66)"?"background:#0d9488;color:#ffffff;border:1px solid #0f766e;":"background:#f1f5f9;color:#334155;border:1px solid #e2e8f0;"):p==="checked_out"?(g="\u9000\u52E4\u6E08",h="attrec-pill ok",y="background:#dcfce7;color:#166534;border:1px solid #86efac;"):p==="working"||p==="holiday_working"?S?(g="\u9000\u52E4\u5FD8\u308C",h="attrec-pill danger",y="background:#fef2f2;color:#991b1b;border:1px solid #fca5a5;"):(g=p==="working"?"\u51FA\u52E4\u4E2D":"\u4F11\u65E5\u51FA\u52E4\u4E2D",h="attrec-pill warn",y="background:#fef9c3;color:#854d0e;border:1px solid #fde047;"):p==="holiday_work"?(g="\u4F11\u65E5\u51FA\u52E4",h="attrec-pill warn",y="background:#fef9c3;color:#854d0e;border:1px solid #fde047;"):p==="leave"&&ht.has(c)||U?(g=c||"\u4F11\u65E5",h="attrec-pill neutral",y=c==="\u534A\u4F11(\u6709\u7D66)"?"background:#0d9488;color:#ffffff;border:1px solid #0f766e;":"background:#f1f5f9;color:#334155;border:1px solid #e2e8f0;"):p==="off"?(g=c||"\u4F11\u65E5",h="attrec-pill neutral",y="background:#f1f5f9;color:#334155;border:1px solid #e2e8f0;"):p==="leave"?(g=c||"\u4F11\u6687",h="attrec-pill neutral",y=c==="\u534A\u4F11(\u6709\u7D66)"?"background:#0d9488;color:#ffffff;border:1px solid #0f766e;":"background:#f1f5f9;color:#334155;border:1px solid #e2e8f0;"):p==="unregistered"?(g="\u672A\u767B\u9332",h="attrec-pill neutral",y="background:#f8fafc;color:#94a3b8;border:1px solid #e2e8f0;"):p==="checkout_missing"?(g="\u9000\u52E4\u5FD8\u308C",h="attrec-pill danger",y="background:#fef2f2;color:#991b1b;border:1px solid #fca5a5;"):p==="not_punched"?(g="\u672A\u6253\u523B",h="attrec-pill danger",y="background:#fef2f2;color:#991b1b;border:1px solid #fca5a5;"):S?(g="\u6253\u523B\u306A\u3057",h="attrec-pill danger",y="background:#fef2f2;color:#991b1b;border:1px solid #fca5a5;"):(g="\u672A\u51FA\u52E4",h="attrec-pill neutral",y=dateIsToday&&nowHHMM>LATE_THRESHOLD?"background:#fef2f2;color:#991b1b;border:1px solid #fecaca;":"background:#f0f9ff;color:#0369a1;border:1px solid #7dd3fc;");const Bt=U?"":nt(n.attendance?n.attendance.checkIn:void 0),Tt=U?"":nt(n.attendance?n.attendance.checkOut:void 0),Mt=n.report&&n.report.site?n.report.site:"",It=n.report&&n.report.work?n.report.work:"",Y=R=>String(R||"").trim()||"",yt=Y(Bt),wt=Y(Tt),Lt=Y(Mt),Ht=Y(It),J=String(n.workType||(n.report&&n.report.workType?n.report.workType:"")||"").trim(),qt=zt.has(c)?c:J==="onsite"?"\u51FA\u793E":J==="remote"?"\u5728\u5B85":J==="satellite"?"\u73FE\u5834/\u51FA\u5F35":p==="off"?"\u4F11\u65E5":"",Q=document.createElement("tr");Q.className=p==="checked_out"?"attrec-row checkedout":p==="working"||p==="holiday_work"||p==="holiday_working"?"attrec-row working":(p==="leave"&&ht.has(c),"attrec-row absent");const Ut=`<td data-label="\u793E\u54E1\u756A\u53F7" style="text-align:center; vertical-align:middle;">${v(V)}</td>`,Rt=`<td data-label="\u6C0F\u540D" style="font-weight:600; white-space:nowrap; vertical-align:middle;">${v(A)}</td>`,Nt=`<td data-label="\u90E8\u7F72" style="white-space:nowrap; vertical-align:middle;">${v(G)}</td>`,Yt=n.branchName||"",jt=`<td data-label="\u652F\u5E97" style="white-space:nowrap; vertical-align:middle;">${v(Yt)}</td>`,_t=`<td data-label="\u52E4\u52D9\u533A\u5206" style="white-space:nowrap; vertical-align:middle;">${v(qt)}</td>`,Ot=`<td data-label="\u72B6\u614B" style="text-align:center; white-space:nowrap; vertical-align:middle;"><span class="${h}" style="display:inline-block;box-sizing:border-box;min-width:84px;text-align:center;padding:2px 10px;font-size:12px;font-weight:700;line-height:1.3;border-radius:6px;${y}">${v(g)}</span></td>`;Q.innerHTML=`
            ...
            ${_t}
            ${Ot}
            <td data-label="\u51FA\u52E4" style="text-align:center; font-family:monospace; font-size:14px; white-space:nowrap;${yt?(yt>LATE_THR?" color:#c2410c; font-weight:700; background:#fff7ed;":" color:#166534; font-weight:600;"):""}">${v(yt)}${yt&&yt>LATE_THR?`<span style="display:inline-block;margin-left:4px;font-size:10px;font-weight:700;padding:0 4px;background:#fed7aa;color:#c2410c;border-radius:2px;">\u9045\u523B</span>`:""}</td>
            ...
            <td data-label="\u4F5C\u696D\u5185\u5BB9" style="width:auto; min-width:350px; max-width:600px;"><div style="font-size:13px; color:#475569; word-break:break-word; white-space:pre-wrap;">${v(Ht)}</div></td>
          `,I.appendChild(Q)}m.appendChild(I);const q=document.createElement("div");if(q.id="attrecTableWrap",q.className="emp-list-scroll-wrap attrec-list-scroll-wrap",q.style.cssText="overflow:visible; width:100%; margin-bottom:0; position:relative;",q.appendChild(m),e.appendChild(q),iFiltered.length>0){const w=Math.ceil(iFiltered.length/d),n=document.createElement("div");if(a){n.className="attrec-mobile-pagination";const f=document.createElement("div");f.innerHTML=`\u5168 <span style="font-weight:700; color:#0f172a;">${iFiltered.length}</span> \u4EF6\u4E2D <span style="font-weight:700; color:#0f172a;">${L+1}</span> - <span style="font-weight:700; color:#0f172a;">${K}</span> \u4EF6\u3092\u8868\u793A`,f.style.color="#64748b",f.style.fontSize="14px";const x=document.createElement("div");x.style.display="flex",x.style.gap="8px";const $=o===1?"#f1f5f9":"#fff",V=o===1?"not-allowed":"pointer",A=document.createElement("button");A.type="button",A.textContent="\u524D\u3078",A.style.cssText=`padding:6px 12px; border:1px solid #cbd5e1; border-radius:4px; background:${$}; color:#475569; cursor:${V}; font-size:14px;`,A.disabled=o===1,A.onclick=()=>{o>1&&(o--,u(),setTimeout(()=>{e&&e.scrollIntoView({behavior:"smooth",block:"start"})},50))};const G=o===w?"#f1f5f9":"#fff",p=o===w?"not-allowed":"pointer",c=document.createElement("button");c.type="button",c.textContent="\u6B21\u3078",c.style.cssText=`padding:6px 12px; border:1px solid #cbd5e1; border-radius:4px; background:${G}; color:#475569; cursor:${p}; font-size:14px;`,c.disabled=o===w,c.onclick=()=>{o<w&&(o++,u(),setTimeout(()=>{e&&e.scrollIntoView({behavior:"smooth",block:"start"})},50))},x.appendChild(A),x.appendChild(c),n.appendChild(f),n.appendChild(x),e.appendChild(n)}else{n.className="attrec-paging desktop-only",n.style.display="flex",n.style.alignItems="center",n.style.justifyContent="flex-start",n.style.gap="15px",n.style.padding="10px 0 20px 0";const f=document.createElement("button");f.type="button",f.textContent="\u524D\u3078",f.className="pagination-btn",f.disabled=o===1,f.onclick=()=>{o>1&&(o--,u())};const x=document.createElement("button");x.type="button",x.textContent="\u6B21\u3078",x.className="pagination-btn",x.disabled=o===w,x.onclick=()=>{o<w&&(o++,u())};const $=document.createElement("span");$.textContent=`${L+1}-${K} / ${iFiltered.length}`,$.style.fontSize="14px",$.style.color="#333",n.appendChild(f),n.appendChild($),n.appendChild(x),e.appendChild(n)}}};u()}catch(r){if(r&&r.name==="AbortError"||!B)return;e&&(e.innerHTML=`
          <div class="empty-state" style="color:#b00020;">
            <div style="font-size:28px;">\u26A0\uFE0F</div>
            <div>\u8AAD\u307F\u8FBC\u307F\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${v(r&&r.message?r.message:"unknown")}</div>
          </div>
        `)}},C=b.querySelector("#rosterDate"),D=document.getElementById("rosterDateMobile"),pt=async t=>{const e=t.target.value||z;C&&(C.value=e),D&&(D.value=e),await O(e)};C&&C.addEventListener("change",pt),D&&D.addEventListener("change",pt);const _fDeptSel=b.querySelector("#rosterFilterDept"),_fStatusSel=b.querySelector("#rosterFilterStatus");if(_fDeptSel)_fDeptSel.addEventListener("change",()=>{const _tbl=b.querySelector("#rosterTable");if(_tbl){const _ev={target:{value:C&&C.value?C.value:z}};pt(_ev);}});if(_fStatusSel)_fStatusSel.addEventListener("change",()=>{const _tbl=b.querySelector("#rosterTable");if(_tbl){const _ev={target:{value:C&&C.value?C.value:z}};pt(_ev);}});const st=(t,e)=>{const r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(t||"").slice(0,10));if(!r)return t;const i=new Date(Date.UTC(parseInt(r[1],10),parseInt(r[2],10)-1,parseInt(r[3],10)));return isNaN(i.getTime())?t:(i.setUTCDate(i.getUTCDate()+e),i.toISOString().slice(0,10))},ct=()=>D&&D.value||C&&C.value||z,ut=async t=>{t&&(C&&(C.value=t),D&&(D.value=t),await O(t))},mt=b.querySelector("#rosterPrevDayMobile"),bt=b.querySelector("#rosterNextDayMobile");mt&&mt.addEventListener("click",()=>ut(st(ct(),-1))),bt&&bt.addEventListener("click",()=>ut(st(ct(),1)));let M=null;try{M=await vt.get("/api/auth/me")}catch{}M&&M.role==="employee"&&F.querySelectorAll(".excel-dropdown-container, #rosterExportXlsx, #rosterExportXlsxMobile").forEach(t=>{t&&(t.style.display="none")});const P=()=>M&&M.role!=="admin"&&M.role!=="manager"?(alert("\u6A29\u9650\u304C\u3042\u308A\u307E\u305B\u3093\u3002"),!1):!0,ft=b.querySelector("#rosterExportMonthXlsx"),xt=document.getElementById("rosterExportMonthXlsxMobile"),gt=async()=>{if(!P())return;const t=b.querySelector("#rosterMonth"),e=document.getElementById("rosterMonthMobile"),r=t&&t.value?t.value:e&&e.value?e.value:Dt,i=`/api/admin/work-reports/export.xlsx?period=month&month=${encodeURIComponent(r)}`;try{await Z(i,`attendance_month_${r}.xlsx`)}catch(o){alert(String(o&&o.message?o.message:"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F"))}};ft&&ft.addEventListener("click",gt),xt&&xt.addEventListener("click",gt);const _exportBtnBar=document.createElement("div");_exportBtnBar.id="dailyExportBar";_exportBtnBar.style.cssText="display:inline-flex;align-items:center;gap:6px;";const _svgXlsx='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg>';const _svgPdf='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M9 13h1a2 2 0 0 1 0 4H9v-4z"/></svg>';const _Ntt=document.createElement("button");_Ntt.type="button";_Ntt.id="rosterExportDayXlsx";_Ntt.innerHTML=_svgXlsx+" Excel \u51FA\u529B";_Ntt.style.cssText="display:inline-flex;align-items:center;gap:5px;height:30px;padding:0 12px;border:1px solid #86efac;background:#f0fdf4;color:#166534;font-size:12px;font-weight:600;cursor:pointer;border-radius:4px;";const _Ott=document.createElement("button");_Ott.type="button";_Ott.id="rosterExportDayPdf";_Ott.innerHTML=_svgPdf+" PDF \u51FA\u529B";_Ott.style.cssText="display:inline-flex;align-items:center;gap:5px;height:30px;padding:0 12px;border:1px solid #fca5a5;background:#fef2f2;color:#991b1b;font-size:12px;font-weight:600;cursor:pointer;border-radius:4px;";const _Qtt=async(btn,fmt)=>{btn.disabled=!0;const orig=btn.innerHTML;btn.textContent="\u51FA\u529B\u4E2D...";try{const d=ct();const url=`/api/admin/work-reports/export-daily?date=${encodeURIComponent(d)}&format=${fmt}`;const resp=await fetch(url,{credentials:"include"});if(!resp.ok){const e=await resp.json().catch(()=>({}));throw new Error(e.message||`HTTP ${resp.status}`);}const blob=await resp.blob();const ext=fmt==="pdf"?"pdf":"xlsx";const filename=`\u52E4\u6020\u8A18\u9332_${d}.${ext}`;const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=filename;document.body.appendChild(a);a.click();document.body.removeChild(a);setTimeout(()=>URL.revokeObjectURL(a.href),10000);}catch(e){alert("\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F: "+(e&&e.message||e));}finally{btn.disabled=!1;btn.innerHTML=orig;}};_Ntt.onclick=()=>_Qtt(_Ntt,"xlsx");_Ott.onclick=()=>_Qtt(_Ott,"pdf");_exportBtnBar.appendChild(_Ntt);_exportBtnBar.appendChild(_Ott);// Inject sau khi O(z) chạy xong để đảm bảo DOM đã sẵn sàng
setTimeout(()=>{const _summaryEl=b.querySelector("#rosterSummaryInline");if(_summaryEl&&_summaryEl.parentElement){_summaryEl.parentElement.appendChild(_exportBtnBar);}else{const _ctrl=b.querySelector(".attrec-control");if(_ctrl)_ctrl.appendChild(_exportBtnBar);}},0);await O(z);const s=document.createElement("form"),W=new Date(Date.now()+9*3600*1e3).toISOString().slice(0,4);s.style.display="flex",s.style.alignItems="center",s.style.gap="8px";const $t=rt.filter(t=>t.role!=="admin"&&t.role!=="manager");s.innerHTML=`
