import{myPaidBalance as p,applyPaidLeave as v,listMyRequests as g}from"../api/leave.api.js";const n=t=>document.querySelector(t),i=()=>`se.leaveDraft.v1:${String(window.location.pathname||"")}`,m=()=>{try{const t=localStorage.getItem(i()),e=t?JSON.parse(t):null;return e&&typeof e=="object"?e:null}catch{return null}},u=()=>{try{if(!n("#applyForm"))return;const e={savedAt:Date.now(),startDate:String(n("#startDate")?.value||""),endDate:String(n("#endDate")?.value||""),reason:String(n("#reason")?.value||"")};localStorage.setItem(i(),JSON.stringify(e))}catch{}},f=()=>{try{localStorage.removeItem(i())}catch{}};async function y(){const t=n("#balance");t.innerHTML="<div>\u6B8B\u6570\u3092\u8AAD\u307F\u8FBC\u307F\u4E2D\u2026</div>";try{const e=await p(),s=e.grants||[],o=s.map(a=>`<tr><td>${a.grantDate}</td><td>${a.expiryDate}</td><td>${a.daysGranted}</td><td>${a.daysRemaining}</td></tr>`).join("");t.innerHTML=`
      <h3>\u6B8B\u6570</h3>
    ...
    `,await D()}catch(e){t.innerHTML=`<div style="color:#b00">\u53D6\u5F97\u5931\u6557: ${e?.message||"error"}</div>`}}async function D(){const t=n("#applyResult");try{const s=(await g()).map((a,r)=>{const d=`R-${String(r+1).padStart(7,"0")}`,c=a.type||"\u2014",l=String(a.status||"").toLowerCase();return`
        <tr>
